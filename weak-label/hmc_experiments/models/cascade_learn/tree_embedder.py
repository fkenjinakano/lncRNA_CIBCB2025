...
_COMMON_PARAMS = {
    "method": [StrOptions({"all_nodes", "path", "dense_path", "leaves"})],
    "node_weights": [
...
def embed_with_tree(
    ...
    if method == "path":
        ...
        leaves_indexes = tree_estimator.apply(X)
        Xt = np.zeros((X.shape[0],tree_estimator.get_n_leaves()))
        _, mapped_indexes = np.unique(leaves_indexes, return_inverse = True) 
        Xt[range(X.shape[0]), mapped_indexes] = 1
#        print(Xt.shape)
