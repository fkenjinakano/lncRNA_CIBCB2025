...
from .tree_embedder import BaseTreeEmbedder, _hstack
...
#        self.X = X
 #       self.y = y
    ...
    def predict_proba(self, X, hmc = False, **params):
...
class MultiOutputVotingRegressor(_BaseComposition, RegressorMixin):
    ...
    def fit(self, X, y, **params):
        ## X.replace(np.nan_to_num(X.astype(np.float32))) as a walkaround for ValueError: Input X contains infinity or a value too large for dtype('float32').
#        print(type(X))
 #       print(isinstance(X, pd.DataFrame))
        
        if isinstance(X, pd.DataFrame):
            X = X.replace(np.nan_to_num(X.astype(np.float32)))
        else:
            X = np.nan_to_num(X.astype(np.float32))
        #self.estimators_ = [
         #   clone(estimator).fit(X.replace(np.nan_to_num(X.astype(np.float32))), y, **params)
