...
#from scripts.running.run_experiments import load_dataset
#import sys
#from pathlib import Path
#sys.path.append(str(Path(__file__).parents[1].resolve()))
#print(sys.path)

from models import label_complement
...
xt_embedder_leaves_pca = Pipeline([
    ("embedder", xt_embedder_leaves),
#    ("densifier", Densifier()),
    ("pca", PCA(n_components=N_COMPONENTS, random_state=RSTATE)),
])

rf_embedder_leaves_pca = Pipeline([
    ("embedder", rf_embedder_leaves),
#    ("densifier", Densifier()),
    ("pca", PCA(n_components=N_COMPONENTS, random_state=RSTATE)),
])
...
alternating_level_embedding_leaves = AlternatingLevel([
    ("rf", rf_embedder_leaves_pca),
    ("xt", xt_embedder_leaves_pca),
])
...
alternating_level_embedding_leaves_proba = AlternatingLevel([
    ("rf", FeatureUnion([("embedder", rf_embedder_leaves_pca), ("proba", rf_proba_transformer)])),
    ("xt", FeatureUnion([("embedder", xt_embedder_leaves_pca), ("proba", xt_proba_transformer)])),
])
...
cale = clone(Cascade(
    level=alternating_level_embedding_leaves,
    final_estimator=final_estimator,
    **CASCADE_PARAMS,
))

cale_os = clone(Cascade(
    level=alternating_level_embedding_leaves_proba,
    final_estimator=final_estimator,
    **CASCADE_PARAMS,
))
...
#     # cascade = clone(cafe_fla).set_params(
    # data = pd.read_csv("ijcnn/data/MLC/birds.csv")
    # cascade = clone(cafe_slc).set_params(
    #     max_levels=3,
    #     memory=None,
    # )
    # x = data[[c for c in data.columns if "label" not in c]].values
    # y = data[[c for c in data.columns if "label" in c]].values
    # #print(x,y)
